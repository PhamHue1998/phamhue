...
namespace 携帯会社販売管理システム
{
    public class InputCheck
    {
        ...
        // Check Length
        public bool CheckTextLength(string text, int maxLength, int minLength, out string errorMessage)
        {
            errorMessage = string.Empty;
            int check = int.Parse(text);
            if (minLength <= check && check <= maxLength)
                return true;
            else
            {
                errorMessage = string.Format("{0}文字を入力してください！", maxLength);
                return false;
            }
        }
    }
